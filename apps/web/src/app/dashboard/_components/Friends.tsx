...
const Friends = () => {
  ...
  const [actionType, setActionType] = useState<"remove" | "block" | "unblock">(
    "remove",
  );
  ...
  const fetchFriendsData = async () => {
    ...
    try {
      ...
      setFriends(
        friendsResponse.data.friends.filter((f) => f.status === "Accepted") ||
          [],
      );
      setBlockedUsers(
        friendsResponse.data.friends.filter((f) => f.status === "Blocked") ||
          [],
      );
    } catch (error) {
    ...
    try {
      switch (actionType) {
        case "remove":
          ...
          setFriends((prev) =>
            prev.filter((f) => f.friendshipId !== selectedUser.friendshipId),
          );
          toast.success(
            `${selectedUser.displayName || selectedUser.username} removed from friends`,
          );
          break;
        ...
        case "block":
          ...
          toast.success(
            `${selectedUser.displayName || selectedUser.username} has been blocked`,
          );
          break;
        ...
        case "unblock":
          ...
          setBlockedUsers((prev) =>
            prev.filter((u) => u.friendshipId !== selectedUser.friendshipId),
          );
          toast.success(
            `${selectedUser.displayName || selectedUser.username} has been unblocked`,
          );
          break;
      }
    } catch {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium">
                  {friend.user?.displayName || friend.username}
                </p>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <Badge
                  variant="outline"
                  className="bg-red-100 text-red-700 mt-1"
                >
                  Blocked
                </Badge>
              </div>
