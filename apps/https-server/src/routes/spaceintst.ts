...
spaceInstanceRouter.post("/:id/instances", authMiddleware, async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Instance created", instance: newInstance });
    } catch (error) {
...
spaceInstanceRouter.delete("/:id/instances/:instanceId", authMiddleware, async (req, res) => {
    try {
        ...
        if (space.ownerId !== req.user!.id && req.user!.role !== "admin") {
            return res.status(403).json({ error: "Unauthorized to delete this instance" });
        }
