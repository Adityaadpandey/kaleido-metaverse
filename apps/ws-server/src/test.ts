...
const JWT_SECRET =
  ...
  "iamthepapaofmybabaiesuiwsdcverfghbnthjnmpoiuhytgfrsdcvb";
...
function connectClient(user: (typeof testUsers)[0]): Promise<WebSocket> {
  return new Promise((resolve, reject) => {
    ...
    console.log(
      `Connecting user ${user.username} (${user.id}) with token: ${token.substring(0, 15)}...`,
    );
    ...
    ws.on("message", (data) => {
      try {
        ...
        console.error(
          `[${user.username}] Failed to parse message:`,
          data.toString(),
        );
      }
    });
...
async function runTests() {
  ...
  try {
    ...
    const joinResponse1 = await sendAndWait(
      client1,
      joinMessage,
      "joinSpaceConfirm",
    );
    ...
    const joinResponse2 = await sendAndWait(
      client2,
      joinMessage,
      "joinSpaceConfirm",
    );
    ...
    const statusResponse = await sendAndWait(
      client1,
      statusMessage,
      "statusUpdateConfirm",
    );
    ...
    const leaveResponse = await sendAndWait(
      client1,
      leaveMessage,
      "leaveSpaceConfirm",
    );
    ...
    const leaveNotification = await waitForMessage(
      client2,
      "userPresenceUpdate",
    );
