...
export function createWebSocketServer(httpServer: http.Server) {
    ...
    function broadcastToRoom(roomId: string, message: any, exclude?: SocketWithData) {
        const room = rooms.get(roomId);
        if (!room) return;
    ...
    async function handleUpdatePosition(ws: SocketWithData, message: BaseMessage) {
        if (!ws.data) return;

        const { spaceId, instanceId, x, y, z, rotationX, rotationY, rotationZ } = message;
    ...
    async function handleSendChatMessage(ws: SocketWithData, message: BaseMessage) {
        if (!ws.data) return;
